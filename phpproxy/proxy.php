...
$enable_jsonp    = false;
$enable_native   = true;
$valid_url_regex = '/https:\/\/files.*\/events\/.*/';
...
if ( !$url ) {
  ...
  // pass-through for the endTime url parameter.
  ...
  $incomingRequestHeaders = getallheaders();

  if ($incomingRequestHeaders['Authorization']) {
     curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Authorization: ' . $incomingRequestHeaders['Authorization']
      ));
  }
  ...
  // Debugging.
  //var_dump($incomingRequestHeaders);
  //var_dump($status);
...
if ( $_GET['mode'] == 'native' ) {
  ...
  // Header to allow CORS-request, including authorisation.
  header("Access-Control-Allow-Headers: Authorization");
  header("Access-Control-Allow-Origin: *");
